...
class RedGymEnv(Env):
    ...
    def __init__(self,
        ...
        self.act_random_offset = config[
            'action_freq_random_offset'] if 'action_freq_random_offset' in config else 0
    ...
    def reset(self, seed=None):
        ...
        self.recent_frames = np.zeros(
            (self.frame_stacks, self.output_shape[0], self.output_shape[1],
             self.output_shape[2]),
            dtype=np.uint8)
    ...
    def init_knn(self):
        ...
        self.knn_index = hnswlib.Index(
            space='l2',
            dim=self.vec_dim)  # possible options are l2, cosine or ip
    ...
    def run_action_on_emulator(self, action):
        ...
        for i in range(self.act_freq +
                       self.random.randint(0, self.act_random_offset)):
    ...
    def update_reward(self):
        ...
        return (new_step,
                (new_prog[0] - old_prog[0], new_prog[1] - old_prog[1],
                 new_prog[2] - old_prog[2]))

    def group_rewards(self):
        prog = self.progress_reward
        # these values are only used by memory
        return (prog['level'] * 100, self.read_hp_fraction() * 2000,
                prog['explore'] * 160)  #(prog['events'],
    ...
    def create_exploration_memory(self):
        ...
        full_memory = np.stack(
            (make_reward_channel(level), make_reward_channel(hp),
    ...
    def save_and_print_info(self, done, obs_memory):
        ...
        if done:
            ...
            with open(self.s_path / Path(f'all_runs_{self.instance_id}.json'),
                      'w') as f:
    ...
    def get_levels_reward(self):
        ...
        if level_sum < explore_thresh:
            ...
            scaled = (level_sum -
                      explore_thresh) / scale_factor + explore_thresh
    ...
    def get_knn_reward(self):
        ...
        base = (self.base_explore
                if self.levels_satisfied else cur_size) * pre_rew
    ...
    def read_triple(self, start_add):
        return 256 * 256 * self.read_m(start_add) + 256 * self.read_m(
            start_add + 1) + self.read_m(start_add + 2)
